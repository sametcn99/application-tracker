...
import { createCoverLetter, deleteCoverLetter, updateCoverLetter } from "@/shared/lib/cover-letters";
...
const actionSchema = z.object({
	...
	content: z.string().trim().min(1, "validation.required").max(50000, "validation.tooLong"),
});
...
export async function createCoverLetterAction(formData: FormData): Promise<ActionResult> {
	...
	if (!parsed.success) {
		return {
			ok: false,
			error: parsed.error.flatten().fieldErrors.title?.[0] ?? parsed.error.flatten().fieldErrors.content?.[0] ?? "validation.invalid",
		};
	}

	const letter = await createCoverLetter(userId, parsed.data);
	revalidateCoverLetterPaths();
	return { ok: true, data: { id: letter.id, title: letter.title, content: letter.content } };
}

export async function updateCoverLetterAction(id: string, formData: FormData): Promise<ActionResult> {
	const userId = await getCurrentUserId();
	if (!userId) return { ok: false, error: "unauthorized" };
